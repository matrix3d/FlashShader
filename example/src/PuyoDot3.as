...
////v0.2
        ...
        private function draw():void{
            ...
            for(I = 0; I < blobPuyos.length; I++)
            {
				...
				beginFill(g,blobPuyos[I].color);
				g.lineStyle(0);
                moveTo(blobPuyos[I]._VERTS[0].mid.x, blobPuyos[I]._VERTS[0].mid.y);
                for(J = 0;J < blobPuyos[I]._VERTS.length; J++)
                {
                    K = (J+1) % blobPuyos[I]._VERTS.length;
                    curveTo(g,blobPuyos[I]._VERTS[K].x, blobPuyos[I]._VERTS[K].y, blobPuyos[I]._VERTS[K].mid.x, blobPuyos[I]._VERTS[K].mid.y);
                }
				endFill(g);
                for(J = 0;J < blobPuyos[I]._VERTS.length; J++)
                {
		...
		private var lastXY:Point = new Point;
		private var verts:Vector.<Point> = new Vector.<Point>;
		private function moveTo(x:Number, y:Number):void {
			lastXY.x = x;
			lastXY.y = y;
			verts.push(lastXY.clone());
		}
		
		private function curveTo(g:Graphics,cx:Number, cy:Number, tx:Number, ty:Number):void {
			g.moveTo(lastXY.x, lastXY.y);
			g.lineTo(cx, cy);
			g.lineTo(tx, ty);
			g.lineTo(lastXY.x, lastXY.y);
			lastXY.x = tx;
			lastXY.y = ty;
			verts.push(lastXY.clone());
		}
        
		private function beginFill(g:Graphics,color:uint):void {
			g.beginFill(color);
			verts.length = 0;
		}
		private function endFill(g:Graphics):void {
