package gl3d.util
{
	...
	public class Matrix3DUtils extends Object
	{
		...
		public static function decompose(m:Matrix3D,orientationStyle : String = null,vec:Vector.<Vector3D>=null) : Vector.<Vector3D> {
			if(orientationStyle == null) orientationStyle = flash.geom.Orientation3D.EULER_ANGLES;
			if (vec == null) {
				vec = Vector.<Vector3D>([new Vector3D,new Vector3D,new Vector3D]);
			}
			var mr : Vector.<Number> = _raw;
			m.copyRawDataTo(mr);
